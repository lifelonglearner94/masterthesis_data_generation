...
class Blender(core.View):
  ...
  def _add_asset(self, obj: core.FileBasedObject):
    ...
    with RedirectStream(stream=sys.stdout, disabled=self.verbose):  # reduce the logging noise
      with io.StringIO() as fstdout:  # < scratch stdout buffer
        with redirect_stdout(fstdout):  # < also suppresses python stdout
          if extension == "obj":
            # Blender 4.x uses wm.obj_import instead of import_scene.obj
  ...
  def _add_asset(self, obj: core.PrincipledBSDFMaterial):
    ...
    # Blender 4.x renamed/removed some Principled BSDF inputs
    if bpy.app.version >= (4, 0, 0):
      # In Blender 4.x: "Specular" -> "Specular IOR Level"
      if "Specular IOR Level" in bsdf_node.inputs:
        ...
        obj.observe(KeyframeSetter(bsdf_node.inputs["Specular IOR Level"], "default_value"), "specular",
                    type="keyframe")
      # "Specular Tint" removed in Blender 4.x - skip
      # "Transmission Roughness" removed in Blender 4.x - skip
      # "Emission" -> "Emission Color" in Blender 4.x
    ...
    # Blender 4.x: "Transmission" -> "Transmission Weight"
    if bpy.app.version >= (4, 0, 0):
